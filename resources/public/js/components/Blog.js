...
{title:"Embedding my Twitter Timeline", body: "Today I embedded my Twitter timeline to my homepage. There were a few issues I ran into using React concerning the loading order of the script tag Twitter provided, but Reacts componentDidMount and componentWillUnmount solved that problem. Styling the iframe was a bit of extra work, first pulling the original styles out with JS and then setting the values. I also had an issue with the loading order, the fix being that the iframe has to be loaded before I could manipulate the JS. I wrote a var twitterIsFinshedLoading that checks if the iframe is loaded, then an if statement that checks if twitterIsFinshedLoading is true and then inserts the new styles, else it loops every 100 milliseconds until true. It took a bit more time than I expected, but I think it would be a really cool npm package to write, I’ll have to knock that out next."},
